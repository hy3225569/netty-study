...
public class Base64A {
    ...
    public static byte[] decode(String str, int flags) {
        return Base64A.decode(str.getBytes(), flags);
    }

    public static byte[] decode(byte[] input, int flags) {
        return Base64A.decode(input, 0, input.length, flags);
    }
    ...
    public static String encodeToString(byte[] input, int flags) {
        try {
            return new String(Base64A.encode(input, flags), "US-ASCII");
        }
    ...
    public static String encodeToString(byte[] input, int offset, int len, int flags) {
        try {
            return new String(Base64A.encode(input, offset, len, flags), "US-ASCII");
        }
        catch (UnsupportedEncodingException e) {
            throw new AssertionError(e);
        }
    ...
    public static byte[] encode(byte[] input, int flags) {
        return Base64A.encode(input, 0, input.length, flags);
    }
    ...
    extends Coder {
        ...
        public boolean process(byte[] input, int offset, int len, boolean finish) {
            ...
            switch (this.tailLen) {
                ...
                case 1: {
                    if (p + 2 > len) break;
                    v = (this.tail[0] & 255) << 16 | (input[p++] & 255) << 8 | input[p++] & 255;
                    this.tailLen = 0;
                    break;
                }
                case 2: {
